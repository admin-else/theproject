...
const char *mc_error_type_to_str(enum mc_error_type data) {
    switch (data) {
    #define X(data) case data: return #data;
    MC_ERRS_X
    #undef X
    default: return "BAD_ERROR_ID";
    }
}

void print_error_info() {
    printf("%s:%d in ERR%d - %s: %s\n", error.file, error.line, error.type, mc_error_type_to_str(error.type), MC_NULL_SAFE_STR(error.message) );
    print_stack_trace();
}
