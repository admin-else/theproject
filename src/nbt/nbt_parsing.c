 ...
 * -----------------------------------------------------------------------------
 */
#include <mc/nbt/nbt.h>

#include <mc/nbt/buffer.h>
#include <mc/nbt/list.h>
...
//rkr needs a way to advance its memory pointer when reading nbt from protocol
//packets, this is my quick-and-dirty answer --nickelpro
nbt_node* nbt_parse2(const void* mem, size_t len)
{
    errno = NBT_OK;

    size_t* length = &len;

    return parse_named_tag((const char **) mem, length);
}
